...
import { Box, Flex, Heading, IconButton, Logomark, Spinner, Text, VisuallyHidden } from '@sajari-ui/core';
import { Button, Radio, RadioGroup, PoweredBy } from '@sajari/react-components';
...
const App = () => {
  ...
  const renderContent = () => {
    if (searching) {
      return (
        <Flex flexDirection="flex-col" space="space-y-2" alignItems="items-center">
          <Spinner size="xl" />
          <Text>Searching....</Text>
        </Flex>
      );
    ...
    if (results && results.length === 0) {
      return (
        <Flex flexDirection="flex-col" space="space-y-2" alignItems="items-center">
          ...
          <Text>Sorry, we couldn&apos;t find any matches.</Text>
        </Flex>
      );
    }
