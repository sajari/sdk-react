...
export class Result extends React.Component<ResultProps> {
  public render() {
    const { token, values, resultClicked, showImage = false, theme } = this.props;
    ...
    return (
      ...
      >
        {showImage && <Image css={{ display: 'inline-block' }} src={img as string} alt={''} />}
        <div className="sj-result__text" css={{ minWidth: 0 }}>
          ...
          <p
            ...
            css={[resultStyles.description, styles && styles.description && styles.description]}
          >
            {description}
          </p>
          <TokenLink
            ...
            css={[resultStyles.link, styles && styles.url && styles.url, { '&:hover': themeColor(theme) }]}
          >
            {url}
          </TokenLink>
        </div>
