...
export function Summary(props: SummaryProps) {
  ...
  return <SummaryContainer>{(ctx) => <Renderer {...ctx} {...rest} />}</SummaryContainer>;
}
...
function DefaultSummaryRenderer(props: SummaryContext & SummaryProps) {
  ...
  const ariaMessage = `${props.pageNumber} ${props.totalResults} ${i18n.t('summary:resultsFor')} "${props.query}"`;

  return (
    <Container className={classnames('sj-summary', props.className)} styles={styles?.container}>
      <LiveMessage message={ariaMessage} aria-live="polite" />
      <span className="sj-summary__results-text">
        {`${props.pageNumber} ${props.totalResults} ${i18n.t('summary:resultsFor')} `}"
        <Emphasis className="sj-summary__search-term" styles={styles.searchTerm}>
          {props.query}
        </Emphasis>
        "{' '}
      </span>
      {showQueryTime && <span className="sj-summary__query-time">{`(${props.responseTime}) `}</span>}
      {showQueryOverride && (
        <Override
...
const Override: React.SFC<OverrideProps> = ({ className, responseQuery, query, search, styles }) => {
...
const click = ({ search, query }: { search: SearchFn; query: string }) => (event: any) => {
