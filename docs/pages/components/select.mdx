...
function Example() {
  ...
  const handleSubmit = (event) => {
    ...
    setData(new URLSearchParams(data).toString());
  };
...
| `value`     | `OptionProps['value']` \| `Array<OptionProps['value']>` |         | The selected values for the select. Note: if specified, `selected` on the child `<Option>` will be ignored. |
