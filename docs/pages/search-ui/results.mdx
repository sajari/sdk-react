...
Sometimes you'll want to show a collection of images besides the primary one, these are called variant iamges and you can enable them by passing `showVariantImage`.
    ...
    <SearchProvider
      search={{
        ...
        fields: new FieldDictionary({
          ...
          subtitle: (data) => data.level4 || data.level3 || data.level2 || data.level1 || data.brand,
          image: (values) => {
            return new Array(5).fill(values.image);
          },
        }),
        variables: new Variables({
          resultsPerPage: 6,
