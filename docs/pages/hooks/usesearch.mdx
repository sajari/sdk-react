...
function Example() {
  ...
  Object.entries(searches).forEach(([key, { name }]) => {
    searches[key].variables = new Variables({ filter: `brand='${name}'`, resultsPerPage: 4 });
  ...
  const Results = (props) => {
    const { title, variables, pipeline } = props;
    const { results, searching } = useSearch({ variables, pipeline });
      ...
      return (
        <div className="grid md:grid-cols-4 md:gap-4">
          {results.map(({ values }) => {
            ...
            return (
              <article key={id} className="space-y-4 text-center">
  ...
  return (
    <div className="space-y-6">
      {Object.entries(searches).map(([key, { name, variables, pipeline }]) => (
        <Results title={name} pipeline={pipeline} variables={variables} key={key} />
      ))}
    </div>
  );
}
