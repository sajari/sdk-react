...
<SEO
  ...
  description="useRangeFilter returns the filter range (current range and the limit for min and max)"
/>

# useRangeFilter

`useRangeFilter` returns the filter range (current range and the limit for min and max).
...
| `limit`    | `Range`                         | `[0, 0]` | The limit [min, max] value                   |
